...
TX_THREAD comms_thread;
void comms_thread_create(void);
static void comms_thread_func(ULONG thread_input);
static uint8_t comms_thread_stack[1024] BSP_PLACE_IN_SECTION_V2(".stack.comms_thread") BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);
...
static uint8_t queue_memory_g_cdc_queue[24];
...
void comms_thread_create(void)
{
    ...
    err_g_cdc_queue = tx_queue_create (&g_cdc_queue, (CHAR*) "CDC Queue", 3, &queue_memory_g_cdc_queue,
    ...
    UINT err;
    err = tx_thread_create (&comms_thread, (CHAR*) "Comms Thread", comms_thread_func, (ULONG) NULL, &comms_thread_stack,
                            1024, 2, 2, 1, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
        tx_startup_err_callback (&comms_thread, 0);
    }
}
